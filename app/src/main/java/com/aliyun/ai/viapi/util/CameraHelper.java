...
public final class CameraHelper implements Camera.PreviewCallback {
    ...
    private OnCameraPreviewFrameListener mOnCameraListener;
    ...
    public void initCamera() {
        ...
        Log.i(TAG, "frontCameraId:" + mFrontCameraId + ", frontCameraOrientation:"
                + mFrontCameraOrientation + ", backCameraId:" + mBackCameraId + ", backCameraOrientation:"
                + mBackCameraOrientation);
    }

    public void openCamera(Activity activity, int cameraFace) {
        openCamera(activity, cameraFace, DEFAULT_PREVIEW_WIDTH, DEFAULT_PREVIEW_HEIGHT);
    }

    public void openCamera(Activity activity, int cameraFace, int cameraWidth, int cameraHeight) {
        if (mCamera != null) {
            return;
        }
        try {
            this.mCameraFacing = cameraFace;
            boolean isFront = cameraFace == FACE_FRONT;
            ...
            CameraHelper.setFocusModes(parameters);
            CameraHelper.chooseFrameRate(parameters);
            int[] size = CameraHelper.choosePreviewSize(parameters, cameraWidth, cameraHeight);
            mCameraWidth = size[0];
            mCameraHeight = size[1];
            parameters.setPreviewFormat(ImageFormat.NV21);
            CameraHelper.setParameters(mCamera, parameters);

            if (CameraHelper.DEBUG) {
                Map<String, String> fullCameraParameters = CameraHelper.getCameraParameters(mCamera);
    ...
    public void startPreview(SurfaceTexture surfaceTexture) {
        ...
        try {
            ...
            mCamera.stopPreview();
            if (mPreBufferArray == null) {
                mPreBufferArray = new byte[PREVIEW_BUFFER_SIZE][mCameraWidth * mCameraHeight
                        * ImageFormat.getBitsPerPixel(ImageFormat.NV21) / 8];
            }
            // must call after stopPreview

            for (byte[] bytes : mPreBufferArray) {
    ...
    public void setExposureCompensation(float value) {
        setExposureCompensation(mCamera, value);
    }
    ...
    public void handleFocusMetering(float rawX, float rawY, int viewWidth, int viewHeight,
                                    int cameraWidth, int cameraHeight, int areaSize, int cameraFacing) {
        handleFocusMetering(mCamera, rawX, rawY, viewWidth, viewHeight, cameraWidth, cameraHeight, areaSize, cameraFacing);
    }
     ...
     * @param cameraFacing
     */
    public void handleFocusMetering(Camera camera, float rawX, float rawY, int viewWidth, int viewHeight,
                                    int cameraWidth, int cameraHeight, int areaSize, int cameraFacing) {
        if (camera == null) {
            return;
        }
...
//                    return; //cannot autoFocus
    ...
    public void setCameraPreviewFrameListener(OnCameraPreviewFrameListener mOnCameraListener) {
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        if (mOnCameraListener != null) {
            mOnCameraListener.onPreviewFrame(data, camera);
        }
        mCamera.addCallbackBuffer(data);
    }

    public interface OnCameraPreviewFrameListener {
